...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropStr: "<h2>Drag your images here</h2>",
            uploadStr:"Upload",
            abortStr: "Abort",
            cancelStr: "Cancel",
            deletelStr: "Delete",
        ...
        var obj = this;
         ...
         var uploadLabel = $("<div class='btn btn-primary'><i class='glyphicon glyphicon-folder-open'>&nbsp;</i>.. Or browse</div>");
            ...
            if($.fn.ajaxForm) {
                ...
                $(obj).append(obj.errorLog);
   				...
   				if(s.showQueueDiv)
		            ...
		            obj.container = $("<div class='col-xs-12' id='div1'></div>").appendTo($("#filelist"));
	   ...
	   this.startUpload = function () {
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
				...
				if(s.showFileSize)
				fileNameStr += " (<strong>"+getSizeStr(files[i].size)+"</strong>)";
		...
		function defaultProgressBar(obj,s)
		{
			...
			this.statusbar = $("<div class='container uploadWait'></div>");

			//    Tabell     //
			this.table01 = $("<table class='table'></table>").appendTo(this.statusbar);
			//this.tableBody01 = $("<tbody></tbody>").appendTo(this.table01);
			this.tableRow01 = $("<tr></tr>").appendTo(this.table01);

			this.tableItem01 = $("<td class='pull-left'></td>").appendTo(this.tableRow01);
			this.tableItem02 = $("<td class='pull-left text-left'></td>").appendTo(this.tableRow01);
			this.tableItem04 = $("<td class='pull-right'></td>").appendTo(this.tableRow01);

            this.preview = $("<img />").width(s.previewWidth).height(s.previewHeight).appendTo(this.tableItem01).hide();

            this.filename = $("<p class='text-left'></p>").appendTo(this.tableItem02);

            this.progressDiv = $("<div class='ajax-file-upload-progress'>").appendTo(this.tableItem02).hide();

            this.progressbar = $("<div class='ajax-file-upload-bar'></div>").appendTo(this.progressDiv);

            this.abort = $("").appendTo(this.tableItem04).hide();
            this.cancel = $("<button type='button' class='btn btn-danger'><i class='glyphicon glyphicon-trash'> </i> Remove</button>").appendTo(this.tableItem04).hide();
            this.done = $("").appendTo(this.tableItem04).hide();
            this.download = $("").appendTo(this.tableItem04).hide();
            this.del = $("").appendTo(this.tableItem04).hide();
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
            ...
            if(s.autoSubmit) {
                ...
                if(s.showCancel) {
                    pd.cancel.show();
                    pd.cancel.click(function () {
						$( pd.statusbar ).fadeOut( 550,"linear", function() {
							mainQ.splice(mainQ.indexOf(form), 1);
                        removeExistingFileName(obj, fileArray);
                        form.remove();
                        pd.statusbar.remove();
